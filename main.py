...
if train_button:
    ...
    # Path to the main directory containing the clustered face images
    output_dir = 'clustered_faces'

    # List all cluster folders in the directory
    clusters = [folder for folder in os.listdir(output_dir) if os.path.isdir(os.path.join(output_dir, folder))]
    ...
    for cluster in clusters:
        ...
        cluster_folder = os.path.join(output_dir, cluster)
        images = [img for img in os.listdir(cluster_folder) if img.endswith(('png', 'jpg', 'jpeg'))]
        
        # Display images in this cluster
        cols = st.columns(5)  # Adjust the number of columns based on how many images you want per row
